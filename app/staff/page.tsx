...
export default function StaffDashboard() {
  ...
  const crearNuevoModulo = async () => {
    ...
    try {
      ...
      mostrarNotificacion("error", "Error al crear módulo: " + err.message);
    }
  };
  ...
  const descargarExcel = async () => {
    try {
      ...
      mostrarNotificacion("error", "Hubo un problema descargando el Excel. Por favor, intenta de nuevo.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans relative">
      ...
      {notificacion && (
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-6 py-3 rounded-full shadow-xl flex items-center space-x-2 font-bold text-sm animate-in fade-in slide-in-from-top-5 ${notificacion.tipo === "exito" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}>
          {notificacion.tipo === "exito" ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
          <span>{notificacion.mensaje}</span>
        </div>
      )}

      {/* Navbar Superior */}
      <nav className="bg-[#311b42] text-white p-4 shadow-md flex justify-between items-center relative z-40">
        ...
        <div className="flex items-center bg-white/10 rounded-lg px-3 py-1">
          ...
          >
            {modulosList.map(mod => (
              <option key={mod} value={mod} className="text-black">
                {mod}
              </option>
            ))}
          </select>
      ...
      {/* Contenido Principal */}
      <main className="max-w-6xl mx-auto p-4 sm:p-6 mt-4 relative z-10">
        ...
        {vistaActiva === "dashboard" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">Punto de Ingreso</h3>
              <p className="text-gray-600 text-sm mb-6">
                Operando actualmente en el módulo: <strong className="text-gray-900">{moduloSeleccionado}</strong>
              </p>
            ...
            {rolActivo === "Master" && (
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-2">Consolidado Excel</h3>
                <p className="text-gray-600 text-sm mb-6">
                  Descarga los resultados matemáticos de las evaluaciones.
                </p>
                <button 
        ...
        {vistaActiva === "scanner" && (
          <div>
            <h2 className="text-2xl font-extrabold text-gray-900 mb-4">Lector de Ingreso</h2>
        ...
        {vistaActiva === "manual" && (
          <div>
            <h2 className="text-2xl font-extrabold text-gray-900 mb-4">Búsqueda Manual</h2>
        ...
        {vistaActiva === "config" && rolActivo === "Master" && (
          <div className="space-y-6">
            <h2 className="text-2xl font-extrabold text-gray-900 mb-4">Ajustes del Sistema</h2>
            ...
            <div className="w-full max-w-md bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
              ...
              <div className="flex space-x-2">
                <input 
                  ...
                  className="flex-1 border p-3 rounded-xl outline-none focus:ring-2 focus:ring-[#311b42] text-gray-900" 
                />
                <button 
                  onClick={crearNuevoModulo} 
                  className="bg-gray-800 hover:bg-black text-white font-bold px-6 py-3 rounded-xl transition-colors"
                >
                  Crear
                </button>
              </div>
